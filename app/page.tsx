...
export default async function Home() {
  ...
  // else (including anonymous visitors) sees the same plain, non-interactive
  // list as before. Checked here rather than gating a whole separate route,
  // since the list itself is otherwise identical either way.
  const authClient = await createClient();
  const {
    data: { user: viewer },
  } = await authClient.auth.getUser();
  const { data: viewerProfile } = viewer
    ? await authClient.from("profiles").select("role").eq("user_id", viewer.id).maybeSingle()
    : { data: null };
  const canManageKata = ["admin", "organizer", "staff"].includes(viewerProfile?.role ?? "");
                ...
                return (
                  ...
                  >
                    ...
                    <div className="border-t border-neutral-100 px-6 py-4">
                      {open ? (
                        <Link
                          href={`/register?competition=${competition.id}`}
                              ...
                              {kataGroups.map(([base, subCats]) => {
                                ...
                                <details key={base} data-drag-item={base} className="rounded-lg border border-neutral-200 bg-white shadow-sm">
                                  <summary className="flex cursor-pointer items-center gap-2 px-4 py-2.5 text-sm font-semibold text-neutral-800 hover:bg-neutral-50">
                                    {canManageKata ? (
                                      <KataGroupDragZone competitionId={competition.id} base={base} returnTo="/">
                                        <span>
                                      ...
                                      <span className="ml-auto flex flex-wrap gap-1">
                                        {neighborAbove && (
                                          <CategoryActionButton
                                            ...
                                            fields={{ competition_id: competition.id, kata_base: base, direction: "above", return_to: "/" }}
                                        ...
                                        {neighborBelow && (
                                          <CategoryActionButton
                                            actionName="mergeAdjacentKata"
                                            fields={{ competition_id: competition.id, kata_base: base, direction: "below", return_to: "/" }}
                                            className="rounded border border-fuchsia-300 px-2 py-0.5 text-xs font-normal text-fuchsia-700 hover:bg-fuchsia-50"

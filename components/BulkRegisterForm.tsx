        ...
        <table className="w-full min-w-[2000px] text-left">
          <thead className="border-b border-neutral-200 bg-neutral-50 text-xs uppercase tracking-wide text-neutral-500">
            <tr>
              ...
              <th className="px-2 py-2">Date of Birth: DD/MM/YYYY *</th>
            ...
            {rows.map((row, i) => (
              <tr key={i} className="align-top">
                ...
                <td className="px-2 py-1.5">
                  <DateOfBirthField
                    key={`dob-${i}-${rows.length}`}
                    ariaLabel={`Row ${i + 1} date of birth`}
                    defaultValueISO={row.date_of_birth}
                    onISOChange={(iso) => update(i, "date_of_birth", iso)}
                    className={cell}
                  />
                </td>

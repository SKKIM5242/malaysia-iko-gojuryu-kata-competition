...
export default function IssueReportForm() {
  ...
  // Closed by default -- this form is long (10 required fields) and most
  // visits to /account never need it, so it shouldn't compete for space
  // with everything else on the page until someone actually wants it.
  ...
  useEffect(() => {
    if (state.ok) {
      ...
      setScreenSpec("");
      setFileError(null);
      // Nothing more to do once it's sent -- close back up rather than
      // leave a long, now-empty form sitting open.
      setExpanded(false);
    }
  }, [state.ok]);
  ...
  return (
    <section className="mt-8 rounded-lg border border-neutral-300 bg-white p-5">
      ...
      {/* Lives outside the collapsible body -- state.ok closes the form
          back up the moment it succeeds (see the useEffect above), so the
          confirmation has to stay visible on its own or nobody would ever
          see it. */}
      {state.ok && (
        <p className="mt-2 rounded-md border border-green-300 bg-green-50 px-3 py-2 text-sm font-semibold text-green-800">
          {state.ok}
        </p>
      )}

      {!expanded && (
        <p className="mt-1 text-sm text-neutral-500">Click to open the report form. All fields are required.</p>
      )}

 ...
 * clickable "✅ Done" that opens the actual testimonial in a modal, or a
 * plain "⏳ Pending" when the winner hasn't submitted one yet. Always
 * reflects the current database state on every page load (this is a
 * server-rendered table with no caching layer of its own), so it updates
 * the moment a winner submits, not on any kind of daily delay. */
export default function TestimonialStatusCell({
  testimonial,
}: {
  testimonial: { kind: TestimonialKind; mediaUrl: string | null; message: string | null } | null;
}) {
